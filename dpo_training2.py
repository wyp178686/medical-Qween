...
class ScriptArguments:
    ...
    beta: Optional[float] = field(default=0.1, metadata={"help": "The beta parameter for DPO loss."})  # DPO beta参数
...
def main():
    ...
    if args.do_train:
        ...
        train_dataset = tokenized_dataset.filter(
            # NOTE: DPO Trainer handles tokenizing and truncation internally.
            # We filter here for very long examples but rely on the Trainer's max_length.
            lambda x: 0 < len(x['prompt'] + x['chosen']) <= full_max_length * 2  # 增加 *2 冗余避免过于严格的预过滤
                      and 0 < len(x['prompt'] + x['rejected']) <= full_max_length * 2
        )
    ...
    if args.do_eval:
        ...
        eval_dataset = eval_dataset.filter(
            lambda x: 0 < len(x['prompt'] + x['chosen']) <= full_max_length * 2
                      and 0 < len(x['prompt'] + x['rejected']) <= full_max_length * 2
        )
    ...
    training_args = TrainingArguments(  # <--- 已修正: 移除了 max_length, max_prompt_length
    ...
    if args.use_peft:
        ...
        peft_config = LoraConfig(
            ...
            # LoRA Layer Scaling (optional, often helpful)
    ...
    trainer = DPOTrainer(
        ...
        # 修正: 将长度参数放在 DPOTrainer 中
        max_prompt_length=args.max_source_length,  # Prompt 最大长度
        max_length=full_max_length,  # 整个序列 (Prompt + Response) 最大长度
        beta=args.beta,  # DPO 损失的 Beta 参数
        # 优化: 明确截断模式
    ...
    if args.do_train:
        ...
        if trainer.is_world_process_zero():
            ...
            # trainer.model.save_pretrained(args.output_dir) # 这一行与 trainer.save_model 功能重叠，可以移除
